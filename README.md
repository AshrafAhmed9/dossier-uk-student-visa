...
- Firestore-backed demo state and an authenticated Cloud Scheduler recheck.
...
[Open Dossier](https://dossier-console-qupwgyb5aq-uc.a.run.app)

The deployed demo is public and uses one shared, non-personal demonstration
case. Do not enter passport numbers, bank statements, or other personal data.
...
## Deployment and cost guardrail

Cloud Run uses a dedicated Firestore-only runtime identity, zero minimum
instances, and a maximum of one instance. A daily authenticated Cloud Scheduler
job recomputes the morning briefing. The project has a ₹800 monthly budget
alert; alerts are not a spending cap. The deterministic engine and tests remain
free to run locally.
